...
export function useStudyPlan() {
  ...
  const reviewWords = useCallback(async (
    ...
    const reply = await postJson<{ ok: boolean }>(
      listPath(listId) + '/review', { words, action, scope: options?.scope, through: options?.through }
    )
  ...
  return { plan, loading, offline, dueCount, refresh, startWords, reviewWords }
}
